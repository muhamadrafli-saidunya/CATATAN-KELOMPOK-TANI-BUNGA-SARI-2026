...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const calculateArmadaMedaranGroups = (panenRecords: PanenRecord[]) => {
    ...
    return Object.entries(groupMap).map(([groupKey, item]) => {
      // Jumlah kg medaran di monitoring pengangkutan: selisih timbangan kebun vs pabrik (atau potongan pedaran muatan)
      const selisihSusut = Math.max(0, item.totalDimuatKg - item.totalPksKg);
      const medaranKg = Math.max(selisihSusut, item.totalPotonganPedaranKg);
      // Nilai pemasukan kas masuk = Jumlah kg medaran armada × Harga TBS di tanggal periode panen
      const totalMedaranRupiah = medaranKg * item.hargaTbsPerKg;
      return {
        groupKey,
  ...
  // Helper sinkronisasi Buku Kas Kelompok Tani dengan Medaran Armada
  const syncKasWithArmadaMedaran = (currentKasList: KasKelompok[], panenRecords: PanenRecord[]): KasKelompok[] => {
    ...
    armadaGroups.forEach(group => {
      if (group.medaranKg > 0 && group.totalMedaranRupiah > 0) {
        ...
        const kasItem: KasKelompok = {
          ...
          jenis: 'Masuk',
          kategori: 'Medaran',
          keterangan: `Kas Masuk Medaran Armada: Truk ${group.platTruk} (${group.namaSopir}) ke ${group.namaPks} - ${group.medaranKg.toLocaleString('id-ID')} kg @ Rp ${group.hargaTbsPerKg.toLocaleString('id-ID')}/kg`,
          jumlah: group.totalMedaranRupiah,
          saldoSetelah: 0,
